...
| 30 | ~~NLM4 — Network Lock Manager~~ | v3 | ~~XL~~ | ~~Done — synchronous NLM v4 (program 100021): NULL/TEST/LOCK/CANCEL/UNLOCK/FREE_ALL. Shared ByteRangeLockTable provides cross-protocol conflict detection with NFSv4 locks. Registered with portmapper.~~ |
| 31 | ~~NSM integration for NLM~~ | v3 | ~~XL~~ | ~~Done — NSM client (program 100024): SM_MON/SM_UNMON/SM_UNMON_ALL. Discovers rpc.statd port via portmapper. Handles SM_NOTIFY to release locks for rebooted clients. Graceful degradation if statd unavailable.~~ |
